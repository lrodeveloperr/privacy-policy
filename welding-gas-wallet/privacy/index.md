...
The iOS privacy manifest declares no tracking domains and no data types collected by the app. It declares app-only access to UserDefaults for preferences. Data that stays on the device is not sent to GoodUse Studios.
...
## 7. Legal pages and support

When you choose a Privacy, Terms, Support, Data Deletion or Safety link, the app opens the selected HTTPS page in Apple's in-app browser. Apple, GitHub Pages and their infrastructure providers may receive ordinary connection information, such as an IP address, browser or device information, requested page and access time, under their own policies. The app does not attach your cylinder records to those requests.
